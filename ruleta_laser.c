...
typedef struct {
    ...
    const GpioPin* home_pin; // Pin del sensor de home usado para detener el movimiento
    ...
    uint32_t motor_pulse_period_ms; // Intervalo entre grupos de pulsos (ciclos completos)
...
typedef struct {
    uint32_t motor_pulse_count;
    uint32_t motor_pulse_period_ms;
    uint32_t motor_pulse_width_us;
...
static bool load_settings(MotorPulsesContext* ctx) {
    ...
    if(file != NULL && storage_file_open(file, SETTINGS_FILE_PATH, FSAM_READ, FSOM_OPEN_EXISTING)) {
        AppConfig settings;
        size_t bytes_read = storage_file_read(file, &settings, sizeof(settings));
        if(bytes_read == sizeof(settings)) {
            ctx->motor_pulse_count = settings.motor_pulse_count;
            ctx->motor_pulse_period_ms = settings.motor_pulse_period_ms;
            ctx->motor_pulse_width_us = settings.motor_pulse_width_us;
            ctx->total_repetitions = settings.total_repetitions;
            loaded = true;
        }
...
static void save_settings(const MotorPulsesContext* ctx) {
    Storage* storage = furi_record_open(RECORD_STORAGE);
    ...
    if(file != NULL) {
        AppConfig settings = {
            .motor_pulse_count = ctx->motor_pulse_count,
            .motor_pulse_period_ms = ctx->motor_pulse_period_ms,
            .motor_pulse_width_us = ctx->motor_pulse_width_us,
            .total_repetitions = ctx->total_repetitions,
        };

        if(storage_file_open(file, SETTINGS_FILE_PATH, FSAM_WRITE, FSOM_CREATE_ALWAYS)) {
        ...
        if(status == FuriStatusOk) {
            if(event.type == EventTypeInput) {
                ...
                // Lógica de la pantalla de confirmación: iniciar el ciclo si el usuario acepta.
                    ...
                    if(furi_hal_gpio_read(&home_switch_pin)) {
                        FURI_LOG_E(
                            "ruleta_laser",
